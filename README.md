...
| Can't pan/zoom the alpha map (`#!/map2`) by finger | v0.6.2 (`map2TouchAction`): map2 is a modern app2 canvas component (`app-world-map-map`) that handles pan/pinch via pointer events, but the browser was claiming the touch gesture first. Setting `touch-action: none` on its canvas lets the component's own pointer gestures work by finger. (If map2 turns out to be mouse-only, a touch→pointer/mouse bridge is the fallback.) |
