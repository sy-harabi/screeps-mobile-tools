...
| Can't pan/zoom the alpha map (`#!/map2`) by finger | map2 is a modern app2 canvas component (`app-world-map-map`), but its pan/zoom is **mouse + wheel only** (0.6.2 proved touch/pointer does nothing). So v0.6.3 routes map2 through the same touch→mouse pan bridge (`worldMapPan`) and pinch→wheel zoom bridge (`pinchZoomMap`); `touch-action: none` (`map2TouchAction`) hands them the raw touch stream. Note: because a finger drag now pans the map, browser pull-to-refresh over the map area no longer fires (panning and pull-to-refresh can't share the same downward drag). |
